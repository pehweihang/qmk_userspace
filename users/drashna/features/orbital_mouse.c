...
#    define ORBITAL_MOUSE_SPEED_CURVE {24, 24, 24, 32, 58, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66, 66}
//     |               |               |               |           |
...
enum {
    ...
    WHEEL_SPEED_Q2_6 = (ORBITAL_MOUSE_WHEEL_SPEED) < 3.99 ? ((uint8_t)((ORBITAL_MOUSE_WHEEL_SPEED) * 64 + 0.5)) : 255,
    /** Double click delay in units of intervals. */
    DOUBLE_CLICK_DELAY_INTERVALS = (ORBITAL_MOUSE_DBL_DELAY_MS) / (ORBITAL_MOUSE_INTERVAL_MS),
};
...
static const uint8_t init_speed_curve[NUM_SPEED_CURVE_INTERVALS] = ORBITAL_MOUSE_SPEED_CURVE;
static struct {
...
static int16_t scaled_sin(uint8_t amplitude, uint8_t phase) {
    ...
    static const uint8_t lut[NUM_ANGLES / 2] PROGMEM = {0,   25,  50,  74,  98,  120, 142, 162, 180, 197, 212,
    ...
    int16_t value = (int16_t)(((uint16_t)amplitude * pgm_read_byte(lut + (phase & (NUM_ANGLES / 2 - 1))) + 2) >> 2);
...
uint8_t get_orbital_mouse_angle(void) {
    return state.angle & (NUM_ANGLES - 1);
}

void set_orbital_mouse_angle(uint8_t angle) {
    state.x += scaled_sin(RADIUS_Q6_2, state.angle);
    state.y += scaled_cos(RADIUS_Q6_2, state.angle);
    state.angle = angle;
    state.x -= scaled_sin(RADIUS_Q6_2, angle);
    state.y -= scaled_cos(RADIUS_Q6_2, angle);
    wake_orbital_mouse_task();
}

bool process_orbital_mouse(uint16_t keycode, keyrecord_t* record) {
    if (!(IS_MOUSE_KEYCODE(keycode) || (OM_DBLS <= keycode && keycode <= OM_SEL8))) {
        return true;
    }
    ...
    if (state.move_dir) {
        ...
        if (state.move_t <= 16 * (NUM_SPEED_CURVE_INTERVALS - 1)) {
            if (state.move_t == 0) {
                ...
                state.speed += (int16_t)state.speed_curve[i + 1] - (int16_t)state.speed_curve[i];
            }

            ++state.move_t;
        }
        // Round and cast from Q9.6 to Q6.2.
        const uint8_t speed = (state.speed + 8) / 16;

        state.x -= state.move_dir * scaled_sin(speed, state.angle);
        state.y -= state.move_dir * scaled_cos(speed, state.angle);
        active = true;
    }

    // Update heading angle if steering.
    if (state.steer_dir) {
        set_orbital_mouse_angle(state.angle + state.steer_dir);
        active = true;
    }
