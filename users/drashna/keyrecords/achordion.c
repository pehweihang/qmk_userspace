...
enum {
    ...
    STATE_RELEASED,  // Achordion is inactive.
    STATE_TAPPING,   // Active tap-hold key has been settled as tapped.
    STATE_HOLDING,   // Active tap-hold key has been settled as held.
    STATE_RECURSING, // This state is set while calling `process_record()`, which will recursively
...
bool process_achordion(uint16_t keycode, keyrecord_t* record) {
    ...
    // key is released.
    if (!pressed_another_key_before_release && record->event.pressed && tap_hold_keycode != KC_NO &&
